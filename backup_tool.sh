...
function register_strategy {
  local name=$1; shift
  COMPRESS_FUNCS["$name"]=$1; shift
  DECOMPRESS_FUNCS["$name"]=$1; shift
  ENCRYPT_FUNCS["$name"]=$1; shift
  DECRYPT_FUNCS["$name"]=$1; shift
  EXTENSIONS["$name"]=$1
}
...
function make_backup {
  local in=$1 out=$2
  local tmp_dir; tmp_dir="$(mktemp_dir)"; TEMP_FILES+=("$tmp_dir")
...
Options:
  ...
  -s, --strategy NAME      Backup strategy (default: $BACKUP_STRATEGY)
...
function parse_args {
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      -h|--help) usage ;;
      -d|--destination) CUSTOM_OUTPUT_DIR="$2"; shift ;;
      -s|--strategy) BACKUP_STRATEGY="$2"; shift ;;
